...
class CompanyRepository @Inject constructor(
    ...
    fun createCompany(name: String, context: Context, cb: () -> Unit) {
        ...
        companyRef.get().addOnSuccessListener { snapshot ->
            ...
            if (snapshot.exists()) {
                ...
                dbUsersRef.document(FirebaseAuth.getInstance().currentUser!!.uid).set(
                    mapOf("companyname" to name), SetOptions.merge()
                )
        ...
        if (companyName.isNotBlank()) {
            dbCompanyRef.document(companyName).get().addOnSuccessListener { snapshot ->
                ...
                if (!snapshot.exists()) {
                    ...
                    val data = mapOf(
                        "companyname" to companyName, "registered" to true
                    )
    ...
    private fun addUserToCompany(companyName: String, uid: String) {
        ...
        dbCompanyRef.document(companyName).collection("memberIDs").document().set(userObj, SetOptions.merge())
    }
    ...
    fun deleteCompanyAccount(companyName: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            companyRef.collection("memberIDs").get().addOnSuccessListener { snapshot ->
